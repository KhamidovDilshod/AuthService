...
public static class Extensions
{
    ...
    private class CustomNamingConventions : NamingConventions
    {
        ...
        {
            var @namespace = type.GetCustomAttribute<MessageNamespaceAttribute>()?.Namespace ?? defaultNamespace;
            var seperatedNamespace = string.IsNullOrWhiteSpace(@namespace) ? string.Empty : $"{@namespace}";

            return $"{assemblyName}/{seperatedNamespace}{type.Name.Underscore()}".ToLowerInvariant();
        }
    }
