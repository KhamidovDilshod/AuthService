...
public class CorrelationContext : ICorrelationContext
{
    ...
    private CorrelationContext(Guid id, Guid userId, Guid resourceId, string traceId, string spanContext,
        string connectionId, string executionId, string name, string origin, string culture, string resource,
        int retries)
    {
        Id = id;
        UserId = userId;
    ...
    public static ICorrelationContext Create<T>(Guid id, Guid userId, Guid resourceId, string origin,
        ...
        => new CorrelationContext(id, userId, resourceId, traceId, spanContext, connectionId, string.Empty,
            typeof(T).Name, origin, culture,
            resource, 0);
